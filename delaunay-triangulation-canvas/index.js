...
function loadInputData(vertexPosList) {
...
function renderCanvas() {
  ...
  canvasDomContext.clearRect(
    ...
    parameter.workspaceHeight
  );

  renderTriangles(canvasDomContext, globalMeshData);

  renderVertices(canvasDomContext, globalMeshData);
}

function renderVertices(canvasDomContext, meshData) {
  canvasDomContext.fillStyle = colorVertex;

  for (let i = 0; i < meshData.vert.length; i++) {
    let canvas_coord = meshData.vert[i];
    if (isPointVisible(canvas_coord))
      canvasDomContext.fillRect(canvas_coord.x - 2, canvas_coord.y - 2, 4, 4);
  }
}

function renderTriangles(canvasDomContext, meshData) {
  const verts = meshData.vert;
  const triangles = meshData.tri;

  canvasDomContext.fillStyle = colorTriangle;
  canvasDomContext.strokeStyle = colorEdge;
  canvasDomContext.lineWidth = 1;

  for (let itri = 0; itri < triangles.length; itri++) {
    const canvas_coord0 = verts[triangles[itri][0]];
    const canvas_coord1 = verts[triangles[itri][1]];
    const canvas_coord2 = verts[triangles[itri][2]];
    ...
    if (isTriangleVisible(canvas_coord0, canvas_coord1, canvas_coord2)) {
      ...
      canvasDomContext.moveTo(canvas_coord0.x, canvas_coord0.y);
      canvasDomContext.lineTo(canvas_coord1.x, canvas_coord1.y);
      canvasDomContext.lineTo(canvas_coord2.x, canvas_coord2.y);
...
function isPointVisible(p) {
  return (
    p.x >= 0 &&
    p.x < parameter.workspaceWidth &&
    p.y >= 0 &&
    p.y < parameter.workspaceHeight
  );
}
...
gui.add(controllerInfo, 'Plot Vertex');
gui.add(controllerInfo, 'Reset Workspace');
...
function initialize() {
  ...
  canvas_L = Math.min(parameter.workspaceWidth, parameter.workspaceHeight);
